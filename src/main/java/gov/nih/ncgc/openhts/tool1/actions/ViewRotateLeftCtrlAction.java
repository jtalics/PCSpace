...
public class ViewRotateLeftCtrlAction extends Tool1Action {
  private Session session;

  public ViewRotateLeftCtrlAction(final Session session) {
  ...
  @Override
    ...
    try {
      if (session.plotManager.getDim() == 3) {
        session.plotManager.view.rotate(-1, 0, Canvas.NINETY_DEGREES);
      }
  ...
  public void actionPerformed(ActionEvent actionEvent) {
    try {
      if (session.plotter.subject.axes.getDim() == 3) {
        session.plotter.view.rotate(-1, 0, Canvas.NINETY_DEGREES);
      }
