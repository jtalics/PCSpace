...
public class ViewRotateUpShiftAction extends Tool1Action {
  private Session session;

  public ViewRotateUpShiftAction(final Session session) {
    super("Rotate up 45", null);
    this.session = session;
    putValue(MNEMONIC_KEY, KeyEvent.VK_X);
    putValue(ACCELERATOR_KEY,KeyStroke.getKeyStroke(KeyEvent.VK_UP, InputEvent.SHIFT_MASK));
    putValue(ACTION_COMMAND_KEY, getClass().getName());
    putValue(SHORT_DESCRIPTION, "Rotate up 45");
    putValue(LONG_DESCRIPTION, "Rotate up 45");
    setEnabled(true);
  }
