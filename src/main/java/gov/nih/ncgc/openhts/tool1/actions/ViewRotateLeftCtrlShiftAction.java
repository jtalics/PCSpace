...
public class ViewRotateLeftCtrlShiftAction extends Tool1Action {
  private Session session;

  public ViewRotateLeftCtrlShiftAction(final Session session) {
    super("Rotate left 180", null);
    this.session = session;
    putValue(Action.MNEMONIC_KEY, KeyEvent.VK_X);
    putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, InputEvent.CTRL_MASK
        | InputEvent.SHIFT_MASK));
    putValue(Action.ACTION_COMMAND_KEY, getClass().getName());
    putValue(SHORT_DESCRIPTION, "Rotate left 180");
    putValue(LONG_DESCRIPTION, "Rotate left 180");
    setEnabled(true);
  }
