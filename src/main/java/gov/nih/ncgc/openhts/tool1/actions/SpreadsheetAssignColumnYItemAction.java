...
public class SpreadsheetAssignColumnYItemAction extends Tool1Action {
  private final Session session;

  public SpreadsheetAssignColumnYItemAction(final Session session) {
    super("Assign this column to Y-axis", null);
    this.session = session;
    putValue(Action.MNEMONIC_KEY, null/* KeyEvent.VK_A */);
    putValue(Action.ACTION_COMMAND_KEY, getClass().getName());
    putValue(SHORT_DESCRIPTION, "Assign this column to Y-axis");
    putValue(LONG_DESCRIPTION, "Assign this column to Y-axis");
    setEnabled(true);
  ...
  @Override
    ...
    try {
      ...
      session.pointsetManager.assignAxisToSelectedColumnHeadForPointset(
          Axes.Y_AXIS, columnHead, pointsetTable.getPointset());
    }
