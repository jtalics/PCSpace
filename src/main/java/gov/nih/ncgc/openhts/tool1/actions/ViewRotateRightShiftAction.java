...
public class ViewRotateRightShiftAction extends Tool1Action {
  private Session session;

  public ViewRotateRightShiftAction(final Session session) {
    super("Rotate right 45", null);
    this.session = session;
    putValue(Action.MNEMONIC_KEY, KeyEvent.VK_X);
    putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, InputEvent.SHIFT_MASK));
    putValue(Action.ACTION_COMMAND_KEY, getClass().getName());
    putValue(SHORT_DESCRIPTION, "Rotate right 45");
    putValue(LONG_DESCRIPTION, "Rotate right 45");
    setEnabled(true);
  }
